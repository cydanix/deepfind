...
enum GenericHelper {
    ...
    static func getAppSupportDirectory() -> URL {
        ...
        let bundleID = Bundle.main.bundleIdentifier ?? "com.deepfind"
    ...
    static func getAppLocation() -> String {
        ...
        return "/Applications/\(DeepFindAppName).app"
    }
    ...
    static func paste(text: String) -> Bool {
        ...
        // Do not paste if DeepFind is the active app
        if isDeepFindActive() {
            if GenericHelper.logSensitiveData() {
                Logger.log("Paste skipped: DeepFind is frontmost app", log: Logger.general)
            }
            return false
        }
